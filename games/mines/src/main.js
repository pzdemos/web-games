...
td.rk .crown{display:inline-flex; vertical-align:-4px; margin-right:3px}
td.rk .crown svg{width:15px; height:13px; filter:drop-shadow(0 1px 2px rgba(0,0,0,.4))}
td.rk .crown.gold svg{animation:crownGold 2.4s ease-in-out infinite}
td.rk .crown.silver svg{animation:crownSw 3s ease-in-out infinite}
td.rk .crown.bronze svg{animation:crownSw 3.6s ease-in-out infinite}
@keyframes crownGold{0%,100%{transform:translateY(0) rotate(0)}25%{transform:translateY(-1.5px) rotate(-4deg)}50%{transform:translateY(0)}75%{transform:translateY(-1.5px) rotate(4deg)}}
@keyframes crownSw{0%,100%{transform:translateY(0)}50%{transform:translateY(-1.2px)}}
.crown-1{background:linear-gradient(135deg,rgba(216,180,90,.22),transparent);box-shadow:inset 0 0 0 1px rgba(216,180,90,.35);border-radius:4px}
.rank-num-1{color:#ecd391 !important;text-shadow:0 0 10px rgba(216,180,90,.5)}
...
function loadLb(){
  ...
  api('/leaderboard/mines?mode='+lbMode).then(function(d){
    ...
    d.rows.forEach(function(r){
      ...
      html+='<tr'+meCls+'>'+rankHtml+'<td class="user">'+esc(r.username)+'</td><td>'+fmtMs(r.best)+'</td><td>'+r.wins+'</td><td style="color:var(--faint)">'+r.lastDay+'</td></tr>';
    });
    html+='</tbody></table>';
