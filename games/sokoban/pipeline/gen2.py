...
def solve(grid, goals, boxes, player, node_cap=150000, time_cap=1.5):
    t0 = time.time()
...
def main():
    ...
    quotas = [(3, 14), (4, 14), (5, 10)]
    need_total = sum(n for _, n in quotas)
    while sum(len(v) for v in pool.values()) < need_total and time.time() - t0 < 130:
        attempts += 1
        nb = rng.choice([3, 3, 4, 4, 4, 5, 5, 5])
        W = rng.randint(7, 11); H = rng.randint(6, 9)
        wallp = rng.uniform(0.07, 0.16)
    ...
    with open('/tmp/opencode/sokoban/gen_levels_hard.json', 'w') as f:
        json.dump(out, f)
