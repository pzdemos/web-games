...
def solve(grid, goals, boxes, player, node_cap=150000, time_cap=0.8):
    t0 = time.time()
    ...
    q = deque([(frozenset(boxes), min(R0), 0)])
    seen = {(frozenset(boxes), min(R0))}
    while q:
        bxs, pseed, pushes = q.popleft()
        if all(b in goals for b in bxs):
            return pushes
        ...
        for bx in bxs:
            for dr, dc in dirs:
                f = (bx[0]-dr, bx[1]-dc); t = (bx[0]+dr, bx[1]+dc)
                if f not in R: continue
                if wall(t) or t in bxs: continue
                if corner_dead(t): continue
                nb = set(bxs); nb.discard(bx); nb.add(t)
                nb = frozenset(nb)
                ns = min(reach(f, nb))
                st = (nb, ns)
                if st not in seen:
                    seen.add(st); q.append((nb, ns, pushes+1))
    return None
...
def main():
    ...
    quotas = [(1, 6), (2, 12), (3, 14), (4, 14), (5, 10)]
    need_total = sum(n for _, n in quotas)
    while sum(len(v) for v in pool.values()) < need_total and time.time() - t0 < 150:
        attempts += 1
        nb = rng.choice([1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5])
        W = rng.randint(6, 10); H = rng.randint(6, 9)
        wallp = rng.uniform(0.10, 0.28)
    ...
    with open('/tmp/opencode/sokoban/gen_levels.json', 'w') as f:
        json.dump(out, f)
